...
$page_layouts["creategrn_detailspreview"] = $layout;
...
if($mastertable == "inventorymaster")
{
	$where = "";
...
if($rowcount) 
{
	...
	//	RefNumber - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("RefNumber", $data, $keylink);
			$row["RefNumber_value"] = $value;
			$format = $pSet->getViewFormat("RefNumber");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("RefNumber")))
				$class = ' rnr-field-number';
			$row["RefNumber_class"] = $class;
	//	CurrentStock - Number
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("creategrn", "detailspreview"));
