...
$page_layouts["inventorymaster_pricing_detailspreview"] = $layout;
...
if($mastertable == "creategrn-issue")
{
	$where = "";
...
if($rowcount) 
{
	...
	//	ID1 - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("ID1", $data, $keylink);
			$row["ID1_value"] = $value;
			$format = $pSet->getViewFormat("ID1");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("ID1")))
				$class = ' rnr-field-number';
			$row["ID1_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
...
$xt->load_template(GetTemplateName("inventorymaster_pricing", "detailspreview"));
