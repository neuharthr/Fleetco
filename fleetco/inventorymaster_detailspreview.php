...
$page_layouts["inventorymaster_detailspreview"] = $layout;
...
if($mastertable == "creategrn")
{
	$where = "";
		$formattedValue = make_db_value("ItemID",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$where .= $pageObject->getFieldSQLDecrypt("ItemID") . "is null";
	else
		$where .= $pageObject->getFieldSQLDecrypt("ItemID") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	ItemID - 
			...
			$format = $pSet->getViewFormat("ItemID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("ItemID")))
				$class = ' rnr-field-number';
			$row["ItemID_class"] = $class;
	//	Brand - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("inventorymaster", "detailspreview"));
