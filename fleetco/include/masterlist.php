...
##set @t=@BUILDER.Tables{@dt}##
	...
	##foreach @t.arrMasterTables{@TABLE.strDataSourceTable}.arrMasterKeys as @mk##
		...
		##if IsShowKey(Field(@mk), TYPE_PAGE_LIST)##
...
##if GetViewFormat(@f, TYPE_PAGE_LIST) == FORMAT_FILE ## 
...
##endfor##
	...
	$layout = GetPageLayout("##@TABLE.strShortTableName##", 'masterlist');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');
	
	$xt->displayPartial(GetTemplateName("##@TABLE.strShortTableName##", "masterlist"));
}
