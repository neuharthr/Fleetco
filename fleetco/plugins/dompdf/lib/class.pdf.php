...
//            $tmp_name = "$fbfile.tmp.".uniqid();
            $tmp_name = getabspath("templates_c/font".uniqid());
  ...
  function stream($options = '') {
    ...
    if ( isset($options['pdfDownloadedEnd']) )
      setcookie("pdfDownloadedEnd", $options['pdfDownloadedEnd']);
  ...
  function addText($x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0, $charSpaceAdjust = 0, $smallCaps = false) {
    ...
    if (true || $charSpaceAdjust != 0 || $charSpaceAdjust != $this->charSpaceAdjust) {
  ...
  protected function addImagePngAlpha($file, $x, $y, $w, $h, $byte, $fileInput = false) {
    // generate images
	...
	{
		$imageData = xlinesoft_getFetchedImageData($file);
		$img = imagecreatefromstring( $imageData['imageString'] );
	}
	// @xlinesoft patch end
   ...
   * this should work with remote files
   */
  function addPngFromFile($file, $x, $y, $w = 0, $h = 0, $fileInput = false) {
    ...
    if ( isset($this->imagelist[$file]) ) {
	  ...
	  if(!$fileInput)
	    ...
	    $info = mb_substr($imageData['imageString'], 24, 5, '8bit');
	  //@xlinesoft patch end		
      ...
      if ($is_alpha) { // exclude grayscale alpha
        return $this->addImagePngAlpha($file, $x, $y, $w, $h, $color_type, $fileInput); // @xlinesoft patch
      }
	  ...
	  if( !$fileInput )
		...
		$imgtmp = @imagecreatefromstring( $imageData['imageString'] );
	  //@xlinesoft patch end	
      if (!$imgtmp) {
        return;
      }
  ...
  function addJpegFromFile($img, $x, $y, $w = 0, $h = 0, $fileInput = false) { //@xlinesoft patch
    // attempt to add a jpeg image straight from a file, using no GD commands
    // note that this function is unable to operate on a remote file.

    if (!file_exists($img) && !$fileInput) {
      return;
    }
    ...
    if ( $this->image_iscached($img) ) {
	  ...
	  {
		$imageData = xlinesoft_getFetchedImageData($img);
		$data = $imageData['imageString'];
