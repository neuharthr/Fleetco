  ...
  protected function _image($val) {
    ...
    if ( mb_strpos($val, "url") === false ) {
      ...
      if ( $parsed_url["protocol"] == "" && $this->_stylesheet->get_protocol() == "" ) {
		...
		if( preg_match('/^mfhandler.php?/', $val, $match) )
			$path = $val;
		// @xlinesoft patch end
      }
      else {
...
/*======================*/
  ...
  function set_color($color) {
    ...
    if ( is_null($col) || !is_array($col) ||  !isset($col["hex"]) ) {
      $color = "inherit";
    }
    else {
      $color = $col["hex"];
   ...
   */
  function set_font_size($size) { // $size = "120%" || $size = "12px"
    // @xlinesoft patch begin
	if (strpos($size, "%"))
        $size = round($this->get_font_size() * $size / 100). "px";
   // @xlinesoft patch end
