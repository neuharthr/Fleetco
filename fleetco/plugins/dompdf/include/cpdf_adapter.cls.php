...
class CPDF_Adapter implements Canvas {
   ...
   */
  protected function _convert_gif_bmp_to_png($image_url, $type, $fileInput = false) {
	...
	{
		$imageData = xlinesoft_getFetchedImageData($image_url);
		$im = imagecreatefromstring( $imageData['imageString'] );
    }
	// @xlinesof patch end
  ...
  function image($img, $x, $y, $w, $h, $resolution = "normal") {
    ...
    case IMAGETYPE_JPEG:
      ...
      $this->_pdf->addJpegFromFile($img, $x, $this->y($y) - $h, $w, $h, $fileInput); //@xlinesoft patch
      break;
    ...
    case IMAGETYPE_BMP:
      ...
      $img = $this->_convert_gif_bmp_to_png($img, $type, $fileInput); //@xlinesoft patch
	  $fileInput = false; //@xlinesoft patch
	  
    case IMAGETYPE_PNG:
      if ($debug_png) print '!!!png!!!';

      $this->_pdf->addPngFromFile($img, $x, $this->y($y) - $h, $w, $h, $fileInput); //@xlinesoft patch
      break;
