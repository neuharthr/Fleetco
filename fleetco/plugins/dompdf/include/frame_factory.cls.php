...
class Frame_Factory {
  ...
  static function decorate_frame(Frame $frame, DOMPDF $dompdf, Frame $root = null) {
	...
	// @xlinesoft patch begin. It prevents pseudo elements with display 'table-cell' from using Table_Cell_Frame_Reflower
    if($style->display == "table-cell") 
    {
      $table = Table_Frame_Decorator::find_parent_table($frame);
      if( $frame->get_node()->nodeName == 'dompdf_generated' && ( !$table || !$table->is_table() ) )
          $style->display = "block";
    }
	// @xlinesoft patch end
