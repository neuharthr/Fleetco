...
class Image_Cache {
  ...
  static function resolve_url($url, $protocol, $host, $base_path, DOMPDF $dompdf) {
	...
	if( preg_match('/^mfhandler.php?/', $url, $match) ) 
		$base_path = $dompdf->get_base_path();
	//@xlinesoft patch end	
    ...
    try {
      ...
      // Check is the file is an image
		...
		//@xlinesoft patch begin
		$resolved_url_info = pathinfo($resolved_url); 
        if( $resolved_url_info["basename"] == 'mfhandler.php' )
		{
		  $resolved_url = $url;
		}		
		//@xlinesoft patch end         
        ...
        else if ( $width && $height && in_array($type, array(IMAGETYPE_GIF, IMAGETYPE_PNG, IMAGETYPE_JPEG, IMAGETYPE_BMP)) ) { //@xlinesoft patch        
